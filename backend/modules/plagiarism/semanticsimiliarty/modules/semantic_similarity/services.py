...
class SinhalaTextProcessor:
    ...
    def preprocess(self, text: str) -> str:
        ...
        # keep Sinhala sequences
        sinhala_text = " ".join(self.sinhala_pattern.findall(text))

        # remove non Sinhala chars (keep spaces)
...
class SimilarityDetector:
    ...
    def _multi_ngram_similarity(self, text1: str, text2: str) -> float:
        ngram_scores = []
        for n in [1, 2, 3]:
            ngram_scores.append(self._ngram_similarity(text1, text2, n))
    ...
    def _structural_similarity(self, text1: str, text2: str, tokens1: List[str], tokens2: List[str]) -> float:
        ...
        avg_len_sim = 0.0 if avg_len1 == 0 or avg_len2 == 0 else 1 - abs(avg_len1 - avg_len2) / max(avg_len1, avg_len2)
        ...
        word_len_sim = 0.0 if word_len1 == 0 or word_len2 == 0 else 1 - abs(word_len1 - word_len2) / max(word_len1, word_len2)
    ...
    def _lexical_similarity(self, text1: str, text2: str) -> float:
        ...
        try:
            ...
            return float(jellyfish.jaro_winkler_similarity(text1, text2))
        except Exception:
            return self._simple_string_similarity(text1, text2)
...
class FileHandler:
    ...
    def _decode_text(self, content: bytes) -> str:
        """Decode bytes to string with fallbacks"""
        try:
            return content.decode("utf-8")
        except UnicodeDecodeError:
            # fallback commonly helps if file isn't utf-8
...
# ---------------------------------------------------------------------
# Fine-Tuned Transformer Embedding Service (SentenceTransformer bundle)
# ---------------------------------------------------------------------
...
def download_model_from_huggingface(model_path: str, repo_id: str = HUGGINGFACE_REPO_ID) -> bool:
    ...
    """
    if _is_sentence_transformer_bundle(model_path):
        return True
    ...
    try:
        ...
        if not _is_sentence_transformer_bundle(model_path):
            print("[ERROR] Download finished, but modules.json not found. Repo may not contain a SentenceTransformer bundle.")
            return False
...
class FineTunedEmbeddingService:
    """
    Uses the fine-tuned SentenceTransformer model to compute semantic similarity.
    """
    ...
    # ---------------------------------------------------------------------
# Singleton service (LOAD ONCE)
# ---------------------------------------------------------------------
_embedding_service_singleton = None

def get_embedding_service() -> "FineTunedEmbeddingService":
    global _embedding_service_singleton
    if _embedding_service_singleton is None:
        _embedding_service_singleton = FineTunedEmbeddingService()
