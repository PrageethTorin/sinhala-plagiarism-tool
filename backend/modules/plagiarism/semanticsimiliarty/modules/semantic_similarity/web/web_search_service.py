...
class WebPlagiarismChecker:
    ...
    def _create_search_query(self, text: str, max_length: int = 220) -> str:
        """
    ...
    def search_web(self, text: str, num_results: int = 5) -> List[WebSearchResult]:
        ...
        # Extract content in parallel to reduce wait time on slow sites.
        web_results = []
        max_workers = min(5, max(1, len(search_results)))
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {
                executor.submit(self.content_extractor.extract_content, result["url"]): result
