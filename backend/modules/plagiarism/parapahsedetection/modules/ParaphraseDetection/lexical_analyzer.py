...
# DB guard flags to prevent noisy repeated auth errors during scoring loops.
_DB_STATUS = "unknown"  # unknown | available | unavailable
...
def get_synonyms_from_db(word):
    ...
    """
    synonyms = set()
    global _DB_STATUS, _DB_ERROR_LOGGED

    # If DB is already known unavailable, skip all DB work.
    ...
    try:
        ...
        # Fail open: keep lexical matching running without DB synonyms.
        _DB_STATUS = "unavailable"
        if not _DB_ERROR_LOGGED:
            print(f"[WARN] Synonym DB disabled (using exact-word lexical only): {err}")
    ...
    for word1 in tokens1:
        ...
        for syn in synonyms:
            if syn in temp_tokens2:
                ...
                print(f"   [OK] Synonym matched: '{word1}' == '{syn}'")
                break
