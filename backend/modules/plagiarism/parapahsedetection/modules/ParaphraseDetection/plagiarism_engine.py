...
def process_single_url(url, input_sentences):
    ...
    try:
        ...
        web_sentences = split_sentences(web_raw_content)[:WEB_SCAN_MAX_WEB_SENTENCES]
        ...
        if full_text_direct_score >= DIRECT_SHINGLE_THRESHOLD:
            direct_match_by_index = {
                i: _make_direct_match(s_sent, max(85.0, full_text_direct_score))
        ...
        if not direct_match_by_index:
            for i, s_sent in enumerate(input_sentences):
                ...
                if direct_score >= DIRECT_SHINGLE_THRESHOLD:
                    direct_match_by_index[i] = _make_direct_match(s_sent, max(85.0, direct_score))

        direct_density = len(direct_match_by_index) / len(input_sentences) if input_sentences else 0.0
        if direct_match_by_index and direct_density >= DIRECT_EARLY_DENSITY:
            for i, s_sent in enumerate(input_sentences):
                best_scores.append(
            ...
            return {
                "url": url,
                "overall_paraphrase_percentage": round(sum(best_scores) / len(best_scores), 2),
        ...
        if model is not None and remaining_indexes:
            try:
                remaining_sentences = [input_sentences[i] for i in remaining_indexes]
                input_embeddings = model.encode(remaining_sentences, convert_to_tensor=True)
