...
def main() -> int:
    ...
    paraphrase_port = int(env.get("PARAPHRASE_PORT", "5001"))
    ...
    env["WSA_PORT"] = str(wsa_port)
    env["PARAPHRASE_PORT"] = str(paraphrase_port)
    env["SEMANTIC_PORT"] = str(semantic_port)
    env.setdefault("PARAPHRASE_HOST", "127.0.0.1")
    ...
    try:
        ...
        procs.append((
            ...
            _start_process(
                "WSA",
                [py, "-m", "uvicorn", "backend.modules.plagiarism.writingstyleanalaysis.wsa_api:app", "--host", "127.0.0.1", "--port", str(wsa_port)],
                env,
            ),
        ))
        ...
        procs.append((
            ...
            _start_process(
                "PARA",
                [py, "-m", "backend.modules.plagiarism.parapahsedetection.paraphrase_api"],
                env,
            ),
        ))
        ...
        procs.append((
            ...
            _start_process(
                "SEMA",
                [py, "-m", "uvicorn", "backend.modules.plagiarism.semanticsimiliarty.semantic_api:app", "--host", "127.0.0.1", "--port", str(semantic_port)],
                env,
            ),
        ))
        ...
        procs.append((
            ...
            _start_process(
                "GATE",
                [py, "-m", "uvicorn", "backend.modules.plagiarism.plagiarismdetector.gateway_api:app", "--host", "127.0.0.1", "--port", str(gateway_port)],
                env,
            ),
        ))
