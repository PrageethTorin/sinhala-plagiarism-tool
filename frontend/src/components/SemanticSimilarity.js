...
export default function SemanticSimilarity({ sidebarOpen, setSidebarOpen }) {
  ...
  const handleEnhancedSearch = async () => {
    ...
    try {
      // Web search can take 3-5 minutes (scraping multiple sites with Playwright)
      ...
      if (err.code === 'ECONNABORTED' || err.code === 'ETIMEDOUT') {
        setError("Request timed out. Web search takes a few minutes - please try again.");
  ...
  const buildHighlightedDocument = useCallback((text, matches) => {
    ...
    matches.forEach((match, sourceIndex) => {
      ...
      if (pos !== -1) {
        regions.push({ start: pos, end: pos + userText.length, sourceIndex, match });
        return;
      }

      // Fuzzy match: try to find by sentences/lines
      const lines = userText.split(/\n+/).filter(l => l.trim().length > 20);
      for (const line of lines) {
        const linePos = text.indexOf(line.trim());
        if (linePos !== -1) {
          regions.push({ start: linePos, end: linePos + line.trim().length, sourceIndex, match });
        }
      }
    });
      ...
      <div className="sem-body">
        ...
        <section className="sem-main">
          ...
          <div className="tab-content">
            ...
            {/* ========== PARAGRAPH COMPARISON TAB - hidden for now ========== */}
            {false && activeTab === 'paragraph' && (
